...
# start instance
AMI_ID=$(aws ec2 describe-images --filters "Name=name,Values=*habanalabs-base-ubuntu20.04*1.6*"  --query 'Images[0].ImageId' --output text)
echo "AMI ID: ${AMI_ID}"
...
echo "install dependencies:\n pip install optimum-habana jupyter"
echo "start jupyter session with\n jupyter notebook --port=8888"